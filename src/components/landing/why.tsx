...
export function Why() {
  return (
    <section id="why" className="border-y border-border bg-card/40 py-20 sm:py-24">
      <Container>
        <SectionHeading
        ...
        <div className="mx-auto mt-14 grid max-w-3xl gap-4 sm:grid-cols-2">
          {WHY_POINTS.map((point, i) => {
            const isLastOdd = i === WHY_POINTS.length - 1 && WHY_POINTS.length % 2 !== 0;
            return (
              <Reveal 
                key={point} 
                delay={i * 0.06} 
                className={isLastOdd ? "sm:col-span-2 sm:mx-auto sm:w-fit" : ""}
              >
                <div className={`flex items-start gap-3 rounded-2xl border border-border bg-card p-5 ${isLastOdd ? "items-center" : ""}`}>
